...
def main(args):
    ...
    nonlocal_variables['current_goal_idx'] = args.goal_obj_idx
    ...
    robot.restart_sim() #restrart sim before start
    #print('Before training, restart simulation')
        ...
        while True:
            if nonlocal_variables['executing_action']:
                ...
                if goal_conditioned:
                    ...
                    current_mask = robot.mask(color_heightmap, nonlocal_variables['current_goal_idx'])
                    current_mask = np.float32(current_mask)  # converting cause cv2 handels float32 and not float64

                    obj_grasp_prediction = np.multiply(grasp_predictions, current_mask)
                    grasp_predictions = obj_grasp_prediction / 255

                    # for the test
                    mask_weight = 0.05
                    maskPush = np.float32(robot.mask_all_obj(color_heightmap))
                    push_predictions = (1 - mask_weight) * np.multiply(push_predictions, maskPush) + mask_weight * np.multiply(push_predictions, current_mask)
                    ...
                    if np.max(obj_contour[:, 0]) < 0 or np.max(obj_contour[:, 1]) < 0 or np.min(obj_contour[:, 0]) > 224 or np.min(obj_contour[:, 1]) > 224:
                        ...
                        print("goal object is pushed completely out of scene")
                        no_change_count = [0, 0]
                        robot.restart_sim()
                        robot.add_objects()
                        ...
                        print('object is out of scene!')
                        print('number %d test trials ends'%len(trainer.clearance_log))
                ...
                if stage == 'grasp_only':                   
                    ...
                    if best_grasp_conf > grasp_reward_threshold or nonlocal_variables['push_step'] == max_push_episode_length:
                        ...
                        if(best_push_conf > 1e-4 or best_push_conf < -1e-4):
                            nonlocal_variables['episode_ratio_of_grasp_to_push'] = best_grasp_conf / best_push_conf
                            if(nonlocal_variables['episode_ratio_of_grasp_to_push']  > 100 or nonlocal_variables['episode_ratio_of_grasp_to_push']  < -100):
                                nonlocal_variables['episode_ratio_of_grasp_to_push']  = 100
                        else:
                            nonlocal_variables['episode_ratio_of_grasp_to_push']  = 100
                    ...
                    if is_testing:
                        if not goal_conditioned and not grasp_goal_conditioned:
                            ...
                            if random_scene_testing:
                                ...
                                if nonlocal_variables['border_occupy_ratio'] > 0.08 or best_grasp_conf < 1.8 or 1.5* best_push_conf > best_grasp_conf:
                                    nonlocal_variables['primitive_action'] = 'push'
                                #if nonlocal_variables['border_occupy_ratio'] > 0.1 or best_grasp_conf < 1.8 or 1.5* best_push_conf > best_grasp_conf: #0.3, 1.5,     0.1, 2.5
                                #    nonlocal_variables['primitive_action'] = 'push'
                    else:
                ...
                if heuristic_bootstrap and nonlocal_variables['primitive_action'] == 'push' and no_change_count[0] >= 2:
                    ...
                    if nonlocal_variables['primitive_action'] == 'push':
                        #keo
                        current_mask = np.float32(robot.mask(color_heightmap, nonlocal_variables['current_goal_idx'])) 
                        mask = np.float32(robot.mask_all_obj(color_heightmap))
                        mask_weight = 0.05
                        push_predictions = (1 - mask_weight) * np.multiply(push_predictions, mask) + mask_weight * np.multiply(push_predictions, current_mask)
                        push_predictions = push_predictions / 255
                ...
                if nonlocal_variables['primitive_action'] == 'push': # or nonlocal_variables['primitive_action'] == 'place':
                    ...
                    if stage == 'push_only':
                        if best_grasp_conf <= grasp_reward_threshold and nonlocal_variables['push_step'] < max_push_episode_length:
                            ...
                            if goal_conditioned:
                                prev_occupy_ratio = utils.get_occupy_ratio(current_goal_mask_heightmap, depth_heightmap)      
                ...
                if nonlocal_variables['primitive_action'] == 'push':
                    ...
                    if stage == 'push_only':
                        if best_grasp_conf <= grasp_reward_threshold and nonlocal_variables['push_step'] < max_push_episode_length:
                            ...
                            if grasp_goal_conditioned or goal_conditioned:
                                ...
                                latest_goal_mask_heightmap = robot.mask(latest_color_heightmap, nonlocal_variables['current_goal_idx'])
                            ...
                            if goal_conditioned:
                                ...
                                mask = robot.mask(latest_color_heightmap, nonlocal_variables['current_goal_idx'])
                            ...
                            if not grasp_goal_conditioned:
                                ...
                                nonlocal_variables['improved_grasp_reward'] = improved_grasp_reward[nonlocal_variables['current_goal_idx']]
                    ...
                    nonlocal_variables['grasp_success'], color_image, depth_image, color_height_map, depth_height_map, grasped_object_ind, gripper_success_open = robot.grasp(primitive_position, best_rotation_angle, workspace_limits)                  
                    ...
                    if goal_conditioned:
                        if nonlocal_variables['grasp_success']:
                            if grasped_object_ind == nonlocal_variables['goal_obj_idx']:
                                ...
                                print('A different goal catched! set goal_catched = 0.5')
                                if not is_testing:
                                    #try for new train
                                    nonlocal_variables['new_episode_flag'] = 1
                    ...
                    if(not gripper_success_open):   #bugs
                        print('the gripper cannot open, restart simulation')
                        print('bugs')
                        no_change_count = [0, 0]
                        robot.restart_sim()
                        robot.add_objects()
    ...
    while True:
        ...
        if goal_conditioned:
            ...
            white = np.sum(goal_mask_heightmap == 255)
            print('the mask in heightmap is %d'%white)
            if(white <= 5 and white >= 0):
                objs_position = np.asarray(robot.get_obj_positions())
                objs_position = objs_position[:,2]
                current_goal_idx = np.argmax(objs_position)
                nonlocal_variables['current_goal_idx'] = current_goal_idx
                print('During solving, mask is too small in heightmap, change current goal to: %d'%nonlocal_variables['current_goal_idx'])
            else:
                nonlocal_variables['current_goal_idx'] = goal_object
                print('During solving, mask is big enough in heightmap,  current goal is: %d'%nonlocal_variables['current_goal_idx'])
            
            trainer.current_grasp_obj_log.append(nonlocal_variables['current_goal_idx'])
            logger.write_to_log('current-goal', trainer.current_grasp_obj_log)

            current_goal_mask_heightmap = robot.mask(color_heightmap, nonlocal_variables['current_goal_idx'])
        ...
        writer.add_image('goal_mask_heightmap', cv2.cvtColor(current_goal_mask_heightmap, cv2.COLOR_BGR2RGB), global_step=trainer.iteration, walltime=None, dataformats='HWC')
        logger.save_visualizations(trainer.iteration, current_goal_mask_heightmap, 'mask')
        cv2.imwrite('visualization.mask.png', current_goal_mask_heightmap)
        ...
        if np.sum(stuff_count) < empty_threshold or (no_change_count[0] + no_change_count[1] > 10):
            ...
            print('number %d trials ends'%len(trainer.clearance_log))
            if is_testing and len(trainer.clearance_log) >= max_test_trials:
                exit_called = True # Exit after training thread (backprop and saving labels)
            continue
        ...
        if not exit_called:
            if stage == 'grasp_only' and grasp_explore:
                ...
                if grasp_explore_actions:
                    ...
                    obj_contour = robot.obj_contour(nonlocal_variables['current_goal_idx'])
                    mask = robot.mask(color_heightmap, nonlocal_variables['current_goal_idx'])
                    mask = np.float32(mask)  # converting cause cv2 handels float32 and not float64
                    ...
                    print('Strategy: exploite (exploration probability: %f)' % (grasp_explore_prob))
        ...
        if 'prev_color_img' in locals():
            ...
            if experience_replay and not is_testing:
                ...
                if not grasp_goal_conditioned or sample_primitive_action == 'push':
                    ...
                    np.asarray(trainer.executed_action_log)[0:trainer.iteration,0] == sample_primitive_action_id))
                else:
                    temp_idx1 = np.logical_and(np.asarray(trainer.reward_value_log)[0:trainer.iteration,0] == sample_reward_value, 
                    np.asarray(trainer.grasp_obj_log)[0:trainer.iteration,0] == sample_goal_obj_idx)
                    temp_idx2 = np.logical_and(np.asarray(trainer.executed_action_log)[0:trainer.iteration,0] == sample_primitive_action_id, temp_idx1)
                    temp_idx3 = np.logical_and(np.asarray(trainer.current_grasp_obj_log)[0:trainer.iteration] == nonlocal_variables['current_goal_idx'], temp_idx2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_test_trials', dest='max_test_trials', type=int, action='store', default=20,                help='maximum number of test runs per case/scenario')
