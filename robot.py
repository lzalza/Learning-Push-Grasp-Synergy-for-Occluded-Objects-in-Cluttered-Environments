...
class Robot(object):
    def __init__(self, stage, goal_object, obj_mesh_dir, num_obj, workspace_limits,
        ...
        for i in range(20):
            for j in range(6):
                if(j % 2 == 0):
                    color_threshold[i,j] = color_avg[i,j] - 3 * color_std[i, j // 2]
                else:
                    color_threshold[i,j] = color_avg[i,j] + 3 * color_std[i, j // 2]
    ...
    def add_objects(self):
        ...
        for object_idx in range(obj_number):
            ...
            centered_ratio = 0.9
            drop_x = (1 - centered_ratio) * (self.workspace_limits[0][1] - self.workspace_limits[0][0] - 0.2) * np.random.random_sample() + self.workspace_limits[0][0] + 0.1 + (self.workspace_limits[0][1] - self.workspace_limits[0][0] - 0.2) * centered_ratio * 0.5
            drop_y = (1 - centered_ratio) * (self.workspace_limits[1][1] - self.workspace_limits[1][0] - 0.2) * np.random.random_sample() + self.workspace_limits[1][0] + 0.1 + (self.workspace_limits[1][1] - self.workspace_limits[1][0] - 0.2) * centered_ratio * 0.5
    ...
    def grasp(self, position, heightmap_rotation_angle, workspace_limits):
        ...
        if(grasp_location_margin > 0.3):
            print('grasp location margin is above 0.3')
        print('current max z position is ', temp_max_z_position)
